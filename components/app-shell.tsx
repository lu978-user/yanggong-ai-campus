      ...
      <header className="sticky top-0 z-30 border-b border-blue-100/80 bg-white/90 px-4 py-3 backdrop-blur-xl lg:hidden">
        <div className="flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span>
              <span className="block text-sm font-black text-slate-950">扬工智行</span>
              <span className="block text-[10px] font-bold text-blue-600">AI校园公益服务</span>
            </span>
          </Link>
          <button
            type="button"
            onClick={() => setDrawerOpen(true)}
            className="grid size-10 place-items-center rounded-full border border-blue-100 bg-white text-slate-700 shadow-sm"
            aria-label="打开菜单"
          >
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <button
            ...
            aria-label="关闭菜单背景"
          />
          <aside className="absolute inset-y-0 left-0 w-[86vw] max-w-[330px] overflow-y-auto border-r border-white/70 bg-white/92 px-4 py-5 shadow-2xl backdrop-blur-2xl">
            <div className="mb-4 flex justify-end">
              <button
                type="button"
                onClick={() => setDrawerOpen(false)}
                className="grid size-9 place-items-center rounded-full bg-slate-100 text-slate-700"
                aria-label="关闭菜单"
              >
                <X className="size-4" />
              </button>
...
function SidebarContent({
  ...
  return (
    <>
      ...
      <div className="mt-5 rounded-2xl border border-blue-100 bg-blue-50/80 p-4 lg:absolute lg:bottom-5 lg:left-4 lg:right-4">
        ...
        <button
          type="button"
          className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-blue-100 bg-white/70 px-3 py-2 text-xs font-bold text-slate-600 transition hover:bg-blue-50"
        >
          <span>🌙</span>
          深色模式
        </button>
